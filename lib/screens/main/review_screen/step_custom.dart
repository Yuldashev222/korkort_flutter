...
class StepProgressIndicatorCustom extends StatelessWidget {
  ...
  const StepProgressIndicatorCustom({
  ...
  /// Build the list of [_ProgressStepCustom],
  ...
  List<Widget> _buildSteps(double stepLength) {
    ...
    for (; isLtr ? step < totalSteps : step >= 0; isLtr ? ++step : --step) {
      ...
      final progressStep = _ProgressStepCustom(
        direction: direction,
        padding: padding,
        color: stepColor,
...
class _ProgressStepCustom extends StatelessWidget {
  final Axis direction;
  final double width;
  final double height;
  final Color color;
